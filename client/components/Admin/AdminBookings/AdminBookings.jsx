...
const AdminBookings = ({data, count, page, setPage}) => {
    ...
    return (
        <div className={styles.content}>
            <h1 className={styles.title}>Под заказ</h1>
                ...
                <tbody>
                    {data.map(order=>
                        <tr key={order._id} className={styles.item}>
                            <td className={styles.item_brand}>{order.brand}</td>
                            <td className={styles.item_aroma}>{order.aroma}</td>
                            <td className={styles.item_size}>{order.size}</td>
                            <td className={styles.item_quantity}>{order.quantity}</td>
